...
def K_prox(x, H, transpose=False):
    ...
    width = x.shape[2]
    if transpose:
        return torch.matmul(x.view([-1, c, 1, width**2]),
                            H).view([-1, c, width, width])
    else:
        return torch.matmul(H, x.view([-1, c, width**2,
                                       1])).view([-1, c, width, width])
...
def barycenter_conv2d(input1,
    ...
    if norm_type == 'max':
        ...
        total_max = unsqueeze3(weights) * max1 + unsqueeze3(1. -
                                                            weights) * max2
    ...
    if not proximal:
        ...
        while (err > stopThr and cpt < numItermax):
            ...
            KV1 = K(V1, xi1)
            b = torch.einsum('i,ijkl->ijkl', weights,
                             torch.log(torch.max(stabThr, U1 * KV1)))

            V2 = input2_normalized / torch.max(stabThr, K(U2, xi1))
            KV2 = K(V2, xi1)
            b += torch.einsum('i,ijkl->ijkl', 1 - weights,
                              torch.log(torch.max(stabThr, U2 * KV2)))
    ...
    if norm_type == 'max':
        output = b / (b.max(2, keepdim=True)[0].max(3, keepdim=True)[0] +
                      stabThr) * total_max
    else:
        output = b / (b.sum(2, keepdim=True).sum(3, keepdim=True) +
                      stabThr) * total_sum
    '''
...
def mixup_process(out,
    ...
    if loss_batch is None:
        ...
        for i in range(len(indices)):
            alpha1 = 2 * loss_batch[i] / (loss_batch[i] +
                                          loss_batch[indices][i])
            alpha2 = 2 * loss_batch[indices][i] / (loss_batch[i] +
                                                   loss_batch[indices][i])
    ...
    if coin == 1:
        if box:
            ...
            if not hidden:
                out, ratio = barycenter_conv2d(out,
                                               out[indices],
                                               reg=reg,
                                               weights=lam,
                                               numItermax=itermax,
                ...
                out_pos, ratio = barycenter_conv2d(out_pos,
                                                   ...
                                                   return_alpha=label_inter,
                                                   proximal=proximal)
                out_neg, ratio = barycenter_conv2d(out_neg,
                                                   out_neg[indices],
                                                   reg=reg,
                                                   weights=lam,
                                                   numItermax=itermax,
        ...
        target_reweighted = target_reweighted * ratio.unsqueeze(
            -1) + target_shuffled_onehot * (1 - ratio.unsqueeze(-1))
...
def cut_3d_graph(unary_cost,
                 ...
                 n_iter=-1,
                 algorithm='swap'):
    assert len(unary_cost.shape
               ) == 4, "unary_cost dimension should be 4! for 3d graph model."
    ...
    gc = gco.GCO()
    gc.create_general_graph(channel * height * width, n_labels,
                            energy_is_float)
    gc.set_data_cost(unary_cost.reshape([channel * height * width, n_labels]))

    c_edges_from, v_edges_from, h_edges_from, c_edges_to, v_edges_to, h_edges_to = get_images_edges_cvh(
        channel, height, width)
    v_edges_w = cost_v.flatten()
    assert len(v_edges_from) == len(
        v_edges_w), 'different sizes of edges %i and weights %i' % (
            len(v_edges_from), len(v_edges_w))
    h_edges_w = cost_h.flatten()
    assert len(h_edges_from) == len(
        h_edges_w), 'different sizes of edges %i and weights %i' % (
            len(h_edges_from), len(h_edges_w))
    c_edges_w = cost_c.flatten()
    assert len(c_edges_from) == len(
        c_edges_w), 'different sizes of edges %i and weights %i' % (
            len(c_edges_from), len(c_edges_w))
...
def graphcut_multi(unary1,
    ...
    unary_cost = (
        large_val *
        np.stack([(1 - lam) * unary1 + lam * unary2 + prior[i]
...
def graphcut_multi_float(unary1,
    ...
    if dim == 2:
        ...
        labels = 1.0 - cut_3d_graph(
            unary_cost, pairwise_cost, pw_x, pw_y, pw_c,
            algorithm='swap') / (n_labels - 1)
...
def mixup_box(input1, input2, grad1, grad2, method='random', alpha=0.5):
    ...
    if method == 'random':
        ...
        if rh > 0 and rw > 0:
            grad_conv = torch.nn.functional.conv2d(
                grad2.unsqueeze(1), weight=grad2.new_ones(size=(1, 1, rh, rw)))
        ...
        if rh > 0 and rw > 0:
            grad_conv1 = torch.nn.functional.conv2d(
                grad1.unsqueeze(1), weight=grad1.new_ones(size=(1, 1, rh, rw)))
            grad_min = grad_conv1.view(batch_size, -1).min(1)[1]
            grad_conv2 = torch.nn.functional.conv2d(
                grad2.unsqueeze(1), weight=grad2.new_ones(size=(1, 1, rh, rw)))
            ...
            for idx in range(batch_size):
                ...
                input1[idx, :, x_paste:x_paste + rh, y_paste:y_paste +
                       rw] = input2[idx, :, x_cut:x_cut + rh, y_cut:y_cut + rw]
...
def mixup_graph(input1,
    ...
    if dim == 2:
        ...
        unary1_torch = grad1_pool / grad1_pool.view(
            batch_size, -1).sum(1).view(batch_size, 1, 1)
    elif dim == 3:
    ...
    if warp > 0:
        ...
        dx = warp * torch.tensor(gaussian_filter(
            (random_state.rand(batch_size, block_num, block_num) * 2 - 1),
            sigma,
            mode="constant",
            cval=0),
                                 dtype=torch.float32,
                                 device='cuda') * 2 / block_num
        dy = warp * torch.tensor(gaussian_filter(
            (random_state.rand(batch_size, block_num, block_num) * 2 - 1),
            sigma,
            mode="constant",
            cval=0),
    ...
    if method == 'random':
        ...
        mask = []
        if warp > 0:
            unary1_torch = F.grid_sample(unary1_torch.unsqueeze(1),
                                         grid + grid_offset,
                                         padding_mode="reflection").squeeze(1)
        unary2_torch = unary1_torch[indices]

        input1_pool = F.avg_pool2d(input1 * std + mean, neigh_size)
        if warp > 0:
            input1_pool = F.grid_sample(input1_pool,
                                        grid + grid_offset,
                                        padding_mode="reflection")
        input2_pool = input1_pool[indices]

        if dim == 2:
            pw_x = torch.zeros([batch_size, 2, 2, block_num - 1, block_num],
                               device='cuda')
            pw_y = torch.zeros([batch_size, 2, 2, block_num, block_num - 1],
                               device='cuda')
        elif dim == 3:
            pw_x = torch.zeros([batch_size, 2, 2, 3, block_num - 1, block_num],
                               device='cuda')
            pw_y = torch.zeros([batch_size, 2, 2, 3, block_num, block_num - 1],
                               device='cuda')
        ...
        pw_x[:, 0, 0], pw_y[:, 0, 0] = neigh_penalty(input2_pool,
                                                     input2_pool,
                                                     k,
                                                     dim=dim)
        pw_x[:, 0, 1], pw_y[:, 0, 1] = neigh_penalty(input2_pool,
                                                     input1_pool,
                                                     k,
                                                     dim=dim)
        pw_x[:, 1, 0], pw_y[:, 1, 0] = neigh_penalty(input1_pool,
                                                     input2_pool,
                                                     k,
                                                     dim=dim)
        pw_x[:, 1, 1], pw_y[:, 1, 1] = neigh_penalty(input1_pool,
                                                     input1_pool,
                                                     k,
                                                     dim=dim)
        ...
        pw_x = (pw_x[:, 1, 0] + pw_x[:, 0, 1] - pw_x[:, 1, 1] -
                pw_x[:, 0, 0]) / 2
        pw_y = (pw_y[:, 1, 0] + pw_y[:, 0, 1] - pw_y[:, 1, 1] -
                pw_y[:, 0, 0]) / 2
        ...
        for i in range(batch_size):
            if dim == 2:
                mask.append(
                    graphcut_multi(unary2[i], unary1[i], pw_x[i], pw_y[i],
                                   alpha[i], beta, eta, n_labels))
            elif dim == 3:
                mask.append(
                    graphcut_multi_float(unary2[i],
                                         unary1[i],
                                         pw_x[i],
                                         pw_y[i],
                                         alpha[i],
...
# input1
            ...
            input1 = transport_image(input1, plan, batch_size, t_block_num,
                                     t_size)

            # input2
            plan = mask_transport(1 - mask,
                                  unary2_torch,
                                  eps=t_eps,
                                  t_type=t_type)
            input2 = transport_image(input2, plan, batch_size, t_block_num,
                                     t_size)
    ...
    ratio = torch.tensor(ratio / block_num**2,
                         dtype=torch.float32,
                         device='cuda')
    if dim == 3:
        ratio /= 3.
...
def mask_transport(mask, grad_pool, eps=0.01, t_type='full'):
    ...
    for _ in range(n_iter):
        ...
        plan_win = torch.zeros_like(cost).scatter_(-2, col_best.unsqueeze(-2),
                                                   1) * plan
...
def transport_image(img, plan, batch_size, block_num, block_size):
    input_patch = img.reshape(
        [batch_size, 3, block_num, block_size,
         block_num * block_size]).transpose(-2, -1)
    input_patch = input_patch.reshape(
        [batch_size, 3, block_num, block_num, block_size,
         block_size]).transpose(-2, -1)
    input_patch = input_patch.reshape(
        [batch_size, 3, block_num**2, block_size,
         block_size]).permute(0, 1, 3, 4, 2).unsqueeze(-1)

    input_transport = plan.transpose(-2, -1).unsqueeze(1).unsqueeze(
        1).unsqueeze(1).matmul(input_patch).squeeze(-1).permute(0, 1, 4, 2, 3)
...
def create_val_folder(data_set_path):
    ...
    for img, folder in val_img_dict.items():
        ...
        if os.path.exists(os.path.join(
                path, img)):  # Check if image exists in default directory
...
if __name__ == "__main__":
    create_val_folder('data/tiny-imagenet-200'
                      )  # Call method to create validation image folders
